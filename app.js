(function () {
  ...
  const MOCK = {
    ...
    cron: {
      jobs: [
        { id: 'morning-report', name: '晨報', enabled: true, schedule: '10 7 * * *', state: { lastRunAtMs: Date.now() - 3600000, nextRunAtMs: Date.now() + 82800000 } },
        { id: 'lang-course', name: '語言課程', enabled: true, schedule: '0 8 * * *', state: { lastRunAtMs: Date.now() - 7200000, nextRunAtMs: Date.now() + 72000000 } },
        { id: 'health-course', name: '健康課程', enabled: true, schedule: '30 7 * * *', state: { lastRunAtMs: Date.now() - 5400000, nextRunAtMs: Date.now() + 75600000 } },
        { id: 'calendar-capture', name: '日曆捕捉', enabled: true, schedule: '0 7 * * *', state: {} },
        { id: 'gmail-digest', name: 'Gmail 摘要', enabled: true, schedule: '30 6 * * *', state: {} },
    ...
    tasks: [
      { id: 1, title: '完成 Kairo Web Dashboard', priority: 'HIGH', status: 'active', due: '2026-03-05', agent: 'executor' },
      { id: 2, title: '複習日語 N2 語法', priority: 'HIGH', status: 'pending', due: '2026-03-10', agent: 'main' },
      { id: 3, title: '聯繫投資人 Alex', priority: 'HIGH', status: 'pending', due: '2026-03-15', agent: 'main' },
      { id: 4, title: '閱讀《思考快與慢》第4章', priority: 'MED', status: 'pending', due: null, agent: 'main' },
      { id: 5, title: '週反思 — 上週進展', priority: 'MED', status: 'done', due: '2026-03-01', agent: 'reviewer' },
      { id: 6, title: 'Gmail 設置自動分類規則', priority: 'LOW', status: 'pending', due: null, agent: 'executor' },
      { id: 7, title: '健康打卡：跑步 5km', priority: 'MED', status: 'active', due: '2026-03-02', agent: 'executor' },
      { id: 8, title: '準備月度彙報 PPT', priority: 'HIGH', status: 'pending', due: '2026-03-28', agent: 'main' },
    ],
    ...
    chatHistory: [
      { role: 'assistant', content: '早上好！今日 7:10 晨報已發送。你有 2 個高優先任務需要跟進，日曆上下午有投資人電話。有什麼我可以幫你的嗎？' },
    ],
  };

  // ─── Hash Router ──────────────────────────────────────────────────────────

  const pages = ['dashboard', 'chat', 'tasks', 'capture', 'settings'];
  ...
  function renderPage(page) {
    switch (page) {
      ...
      case 'chat': /* chat is maintained */ break;
      case 'tasks': renderTasks(); break;
      case 'capture': renderCaptureHistory(); break;
  ...
  async function renderDashboard() {
    ...
    if (cfg.mode === 'live' && getAPI()) {
      try { status = await getAPI().getStatus(); } catch (e) { console.warn('getStatus failed:', e); }
      try { usage = await getAPI().getUsageToday(); } catch (e) { console.warn('getUsageToday failed:', e); }
      try { cron = await getAPI().getCronJobs(); } catch (e) { console.warn('getCronJobs failed:', e); }
    }
    ...
    if (tokensEl) {
      tokensEl.textContent = totalTokens >= 1000
        ? `${(totalTokens / 1000).toFixed(1)}k`
        : String(totalTokens);
    }
    const cost = (usage.total && usage.total.cost) || 0;
    const subTokensEl = document.getElementById('ms-tokens');
    if (subTokensEl) subTokensEl.textContent = `$${cost.toFixed(2)} 今日花費`;

    // Agents card
    const agents = status.agents || [];
    const onlineCount = agents.filter(function (a) { return a.online; }).length;
    const agentsEl = document.getElementById('mv-agents');
    if (agentsEl) agentsEl.textContent = `${onlineCount}/${agents.length}`;
    const dotsEl = document.getElementById('ms-agents');
    if (dotsEl) {
      dotsEl.innerHTML = agents.map(function (a) {
    ...
    const highPriority = tasks.filter(function (t) { return t.priority === 'HIGH' && t.status !== 'done'; });
    const tasksEl = document.getElementById('mv-tasks');
    if (tasksEl) tasksEl.textContent = String(highPriority.length);
    const doneCount = tasks.filter(function (t) { return t.status === 'done'; }).length;
    const subTasksEl = document.getElementById('ms-tasks');
    if (subTasksEl) subTasksEl.textContent = `${Math.round(doneCount / tasks.length * 100)}% 完成率`;

    // Cron card
    const jobsArr = cron.jobs || [];
    const activeJobs = jobsArr.filter(function (j) { return j.enabled !== false; }).length;
    const cronEl = document.getElementById('mv-cron');
    if (cronEl) cronEl.textContent = String(activeJobs);
    const nextJob = jobsArr
      .filter(function (j) { return j.state && j.state.nextRunAtMs; })
      .sort(function (a, b) { return a.state.nextRunAtMs - b.state.nextRunAtMs; })[0];
  ...
  function renderCalendarCards() {
    ...
    container.innerHTML = events.slice(0, 3).map(function (ev) {
  ...
  function renderTaskHighlights() {
    ...
    if (!container) return;
    const high = MOCK.tasks.filter(function (t) {
      return t.priority === 'HIGH' && t.status !== 'done';
    }).slice(0, 3);

    container.innerHTML = high.map(function (task) {
      return `
        <div class="list-card">
  ...
  function renderTasks() {
    ...
    if (taskFilter === 'high') tasks = tasks.filter(function (t) { return t.priority === 'HIGH'; });
    else if (taskFilter === 'active') tasks = tasks.filter(function (t) { return t.status === 'active'; });
    else if (taskFilter === 'done') tasks = tasks.filter(function (t) { return t.status === 'done'; });
  ...
  function renderCaptureHistory() {
    ...
    container.innerHTML = history.slice(-10).reverse().map(function (item) {
  ...
  function renderSettings() {
    ...
    if (cfg.mode === 'live') {
      if (dot) { dot.classList.add('online'); dot.classList.remove('error'); }
      if (text) text.textContent = `已連線: ${cfg.serverUrl}`;
    } else {
      if (dot) { dot.classList.remove('online'); dot.classList.remove('error'); }
      if (text) text.textContent = '未連線 (Demo 模式)';
    }
  ...
  async function sendChatMessage(text) {
    ...
    if (cfg.mode === 'live' && getAPI()) {
      try {
        ...
        const reply = (result && result.choices && result.choices[0] && result.choices[0].message && result.choices[0].message.content) || '（無回覆）';
  ...
  function setupEventListeners() {
    ...
    if (testBtn) {
      testBtn.addEventListener('click', async function () {
        ...
        try {
          ...
          saveConfig();
          if (dot) { dot.classList.add('online'); dot.classList.remove('error'); }
          if (statusText) statusText.textContent = `✅ 已連線: ${url} (v${(status && status.version) || '?'})`;
        } catch (e) {
          if (dot) { dot.classList.add('error'); dot.classList.remove('online'); }
          if (statusText) statusText.textContent = `❌ 連線失敗: ${e.message}`;
        } finally {
    ...
    if (saveSettingsBtn) {
      saveSettingsBtn.addEventListener('click', function () {
        const urlInput = document.getElementById('server-url');
        const tokenInput = document.getElementById('auth-token');
        if (urlInput) cfg.serverUrl = urlInput.value.trim();
        if (tokenInput) cfg.token = tokenInput.value.trim();
    ...
    window.addEventListener('resize', function () {
      ...
      resizeTimer = setTimeout(function () {
        if (currentPage === 'dashboard') drawUsageChart();
      }, 150);
    });
  }
